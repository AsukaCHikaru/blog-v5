...
export const postList: PostSummary[] = ${JSON.stringify(notionPageList)};`
    ...
    const rawPostListPage = await readFileSync(resolve(__dirname, '../templates', 'PostListPage.tsx'), {encoding: 'utf-8'})    
    const postListPage = rawPostListPage
    .replace("//template_import_post_link", "import { PostLink } from '../components/PostLink';")
    .replace("//template_import_data", "import { postList } from '../data';")
    .replace("//template_map_post_link", `{postList.map((postSummary) => <PostLink key={postSummary.pathname} postSummary={postSummary} />)}`);
    
    await writeFileSync(resolve(__dirname, '../src', 'pages', 'PostListPage.tsx'), postListPage)
